...
saveDirectory   = fullfile(thisDirectory,'Cullen2018_R0_Fimbria');
...
for k = 1:2
    ...
    [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_sham_' num2str(par.sim.temp) 'C.mat']));
    if k == 1
    ...
    [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_long_' num2str(par.sim.temp) 'C.mat']));
    ...
    for psw = [0 20]
        ...
        [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_CTRL_psw_' num2str(psw) '_' num2str(par.sim.temp) 'C.mat']));
        if k == 1  && psw == 0
    ...
    for psw = [0:0.2:1.6 2:6 6.477 7:8 8.487 9 10:2:14 15 20]
        ...
        par                         = CalculateNumberOfMyelinLamellae(par, 'max');
        %         par                         = UpdateInternodePeriaxonalSpaceWidth(par, par.myel.geo.peri.value.ref/2, [], [1, 2, 51, 52], 'min');
        [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_psw_' num2str(psw) '_' num2str(par.sim.temp) 'C.mat']));
    ...
    %% Run long node simulations.
    par.node.geo.length.value.ref       = 0.8668;
    ...
    [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_shortnode_' num2str(par.sim.temp) 'C.mat']));
    if k == 1
    ...
    par.myel.geo.gratio.value.ref       = 0.7419;
    par.myel.geo.gratio.value.vec_ref   = par.myel.geo.gratio.value.ref * ones(par.geo.nintn, par.geo.nintseg);
    par.myel.geo.peri.value.ref         = 6.709;
    par.myel.geo.peri.value.vec         = par.myel.geo.peri.value.ref * ones(par.geo.nintn,par.geo.nintseg);
    par.myel.geo.period.value           = 1000*(par.node.geo.diam.value.ref/par.myel.geo.gratio.value.ref-par.node.geo.diam.value.ref-2*par.myel.geo.peri.value.ref/1000)/(2*6.5);
    par                                 = CalculateNumberOfMyelinLamellae(par, 'max');
    %     par                                 = UpdateInternodePeriaxonalSpaceWidth(par, par.myel.geo.peri.value.ref/2, [], [1, 2, 51, 52], 'min');
    [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_altmyelin_' num2str(par.sim.temp) 'C.mat']));
    if k == 1
    ...
    par.node.seg.geo.length.value.vec   = repmat(par.node.geo.length.value.vec / par.geo.nnodeseg, 1, par.geo.nnodeseg);
    par =                                 CalculateLeakConductance(par);
    [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model(par, fullfile(saveDirectory, ['Cullen2018Fimbria_iTBS_' num2str(par.sim.temp) 'C.mat']));
    if k == 1
